...
//----------------------------------------Search Data ----------------------------------------
            ...
            document.getElementById("searchN").addEventListener("input", async function () {
                let inputVal = this.value;
                displayMain(inputVal);
            })
            document.getElementById("searchF").addEventListener("input", async function () {
                let inputVal = this.value;
        displayMain(inputVal);
    })
}
...
async function displayCategories() {
    ...
    for (let i = 0; i < cates.length; i++) {
        ...
        container +=`
        <div class="text-bg-dark col-md-3 mainCol cursor-pointer">
        <div class="main-card" id="${cates[i].strCategory}">
                        <img src="${cates[i].strCategoryThumb}" class="w-100" alt="...">
                        <div class="card-ovlay-categore ">
                        <h5 class="mainCategory">${cates[i].strCategory}</h5>
                        <p>${cates[i].strCategoryDescription.split( " " , 7).join(" ")}</p>
                        </div>
                        </div>
                        </div>
                        `
    ...
    document.getElementById("mainDiv").innerHTML = container;
    document.querySelectorAll(".main-card").forEach((e) => e.addEventListener("click", async function () {
    let  curent = this.getAttribute("id");
        let data = await filterCategory(curent);
        console.log(curent);
...
async function displayArea () {
    ...
    document.getElementById("mainDiv").innerHTML = container;
    document.querySelectorAll(".mainCol").forEach((e) => e.addEventListener("click", async function () {
    let  curent = this.getAttribute("id");
...
async function displayThemealdb () {
    ...
    for (let i = 0; i <= 19; i++) {
        container +=`
        <div class="text-bg-dark col-md-3 mainCol" id="${meals[i].strIngredient}">
        <div class="area-card cursor-pointer">
        <div class="rounded -2 text-center">
                ...
                if (/^\d{11}$/.test(phoneNuper.value)) {
                    ...
                    if (/^\d+$/.test(userAge.value)) {
                        ...
                        if (/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/.test(password.value)) {
                            ...
                            inValed("passwordAlert" , "Please enter a password that is at least 8 characters long and includes at least one uppercase letter, one lowercase letter, and one digit.")
                        }
                    } else {
                        inValed("ageAlert", "Please enter a valid age using only numbers (0-9). The age should be a whole number without any additional characters.")
                    }
                } else {
                    inValed("phoneAlert", "Please enter a 10-digit phone number using only numbers (0-9). Ensure there are no spaces or special characters.")
                }
            } else {
                inValed("emailAlert", "* Please enter a valid email address in the format example@example.com.")
            }
        } else {
            inValed("nameAlert", "* Please enter a username that consists of 3 to 16 characters, using only letters, numbers, hyphens, and underscores.")
        }
    }
